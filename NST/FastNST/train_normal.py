...
RUN_ROOT   = "evaluate/v3/baseline/train"
...
def compute_style_grams(style_path, device, vgg, layer_cfg):
    ...
    for f in files:
        ...
        for layer in layer_cfg["style"]:
            ...
            if acc[layer] is None:
                acc[layer] = g
            else:
                acc[layer] += g
        count += 1
    for layer in acc:
        acc[layer] = (acc[layer] / count).detach()
...
def evaluate_on_val(model, val_loader, vgg, layer_cfg, style_grams, device):
    ...
    with torch.no_grad():
        for combined, rgb in val_loader:
            ...
            tv_loss = total_variation_loss(gen, weight=TV_WEIGHT)

            total_loss = (CONTENT_WEIGHT * c_loss) + (STYLE_WEIGHT * s_loss) + tv_loss

            val_psnr += psnr(gen, rgb).item()
            val_ssim += ssim(gen, rgb).item()
            val_gram += gram_distance(
                gram_matrix(gen_feats[layer_cfg["style"][0]]),
                gram_matrix(rgb_feats[layer_cfg["style"][0]])
            ).item()
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_metrics[f"epoch_{epoch}"] = {"train": train_metrics_epoch, "val": val_metrics}
